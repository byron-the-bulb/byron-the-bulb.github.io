...
// Track the currently loaded JSON URL and its base directory (for resolving CSV paths)
let currentJSONUrl = null;
let currentAnalysisBaseUrl = '';

// Reset all dynamic state so a new analysis paints from a clean slate
function resetViewerState(opts) {
    const { stopAudio = true, resetZoomPan = true, clearUI = true } = opts || {};
    try {
        if (stopAudio && window.chirpletAudio && typeof chirpletAudio.stopAll === 'function') {
            chirpletAudio.stopAll();
        }
    } catch(e) { console.warn('stopAll error (ignored):', e); }
    ...
    if (resetZoomPan) {
        ...
        const panEl = document.getElementById('panX');
        const panValEl = document.getElementById('panXValue');
        if (zoomEl) { zoomEl.value = '1'; }
        if (panEl) { panEl.value = '0'; }
        if (typeof updateZoom === 'function') updateZoom();
        if (typeof updatePan === 'function') updatePan();
        if (zoomValEl) zoomValEl.textContent = '1.0x';
        if (panValEl) panValEl.textContent = '0';
    }

    // Clear UI panels
    if (clearUI) {
        const toggles = document.getElementById('chirpletToggles');
        if (toggles) toggles.innerHTML = '';
        const infoDiv = document.getElementById('analysisInfo');
        if (infoDiv) infoDiv.textContent = 'Loading analysis...';
    }
}
...
// --- Dropdown-driven JSON loader ---
function deriveBaseUrl(url) {
    const idx = url.lastIndexOf('/')
    return idx >= 0 ? url.substring(0, idx + 1) : '';
}

async function initAnalysisDropdown() {
    const select = document.getElementById('analysisSelect');
    const refreshBtn = document.getElementById('reloadAnalyses');
    if (!select) return;

    await populateAnalysisDropdown();

    select.addEventListener('change', async () => {
        const url = select.value;
        if (!url) return;
        await loadAnalysisFromUrl(url);
    });
...
async function populateAnalysisDropdown() {
    ...
    ph.value = '';
    ph.textContent = '-- Select an analysis JSON --';
    select.appendChild(ph);

    // Read list from the JSON script tag once per page load
    if (!window.ANALYSIS_JSON_LIST) {
        const tag = document.getElementById('analysis-json-list');
        if (tag && tag.textContent) {
            try {
                window.ANALYSIS_JSON_LIST = JSON.parse(tag.textContent);
            } catch (e) {
                console.warn('Failed to parse analysis-json-list script content', e);
                window.ANALYSIS_JSON_LIST = [];
            }
        } else {
            window.ANALYSIS_JSON_LIST = [];
        }
    ...
    const list = (window.ANALYSIS_JSON_LIST || []).slice();
    const items = [];

    // Fetch each JSON to read its title
    await Promise.all(list.map(async (entry) => {
        try {
            ...
            const title = (data && data.title) ? String(data.title) : url.split('/').pop();
...
async function loadAnalysisFromUrl(url) {
    try {
        // Reset and stop any ongoing playback; prepare UI for fresh load
        resetViewerState({ stopAudio: true, resetZoomPan: true, clearUI: true });
        const resp = await fetch(url, { cache: 'no-cache' });
        if (!resp.ok) {
            alert('Failed to fetch JSON: ' + url);
            return;
        }
        const data = await resp.json();
        analysisData = data;
        currentJSONUrl = url;
        currentAnalysisBaseUrl = deriveBaseUrl(url);
        // Attempt to load CSV using improved resolver
        await loadCSVFile(analysisData.source_file);
    } catch (e) {
        console.error('Error loading analysis JSON', e);
        alert('Error loading analysis JSON. See console for details.');
    }
}
    ...
    if (window.chirpletAudio) {
        ...
        // Always play only visible chirplets (UI checkbox removed)
        try { chirpletAudio.setPlayVisibleOnly(true); } catch(e) {}
    ...
    // Initialize dropdown selector for JSON analyses, if present
    try { initAnalysisDropdown(); } catch (e) { console.warn('initAnalysisDropdown error', e); }
}
...
function drawNoDataMessage() {
    ...
    textSize(24);
    text('Select an analysis from the dropdown to visualize', width/2, height/2);
}
...
// Manual JSON loader removed in favor of dropdown-driven selection.

async function loadCSVFile(csvPath) {
    // Try several candidate URLs for the CSV
    const candidates = [];

    const isHttp = /^(https?:)?\/\//i.test(csvPath);
    if (isHttp) {
        candidates.push(csvPath);
    } else {
        const filename = csvPath.split('/').pop();
        // 1) Relative to the JSON file's directory
        if (currentAnalysisBaseUrl) {
            candidates.push(currentAnalysisBaseUrl + csvPath);
            if (filename && filename !== csvPath) {
                candidates.push(currentAnalysisBaseUrl + filename);
            }
        }
        // 2) A known data folder under assets
        if (filename) {
            candidates.push('/assets/data/2025-09-21/' + filename);
        }
        // 3) As-is relative to site root
        candidates.push(csvPath.startsWith('/') ? csvPath : ('/' + csvPath));
    }
    ...
    for (const url of uniqueCandidates) {
        try {
            ...
            console.warn('Fetch CSV failed with status', resp.status, 'for', url);
        } catch (e) {
            console.warn('Fetch CSV threw error for', url, e);
        }
    }
...
function updateAnalysisInfo() {
    const infoDiv = document.getElementById('analysisInfo');
    if (analysisData) {
        const fmt = (v, d=3) => (typeof v === 'number' ? Number(v).toFixed(d) : String(v));
        const countSteps = (min, max, step) => {
            if (typeof min !== 'number' || typeof max !== 'number' || typeof step !== 'number' || step <= 0) return null;
            // Tolerant to floating rounding
            const n = Math.floor((max - min) / step + 1e-9) + 1;
            return n > 0 ? n : null;
        };

        let html = `
            <strong>Title:</strong> ${analysisData.title || '(untitled)'}<br>
        ...
        if (typeof analysisData.num_chirplets === 'number') {
            html += `<strong>Requested Chirplets:</strong> ${analysisData.num_chirplets}<br>`;
        }
        if (typeof analysisData.residual_threshold === 'number') {
            html += `<strong>Residual Threshold:</strong> ${fmt(analysisData.residual_threshold, 6)}<br>`;
        }

        const pr = analysisData.param_ranges || null;
        if (pr) {
            const ntc = countSteps(pr.tc_min, pr.tc_max, pr.tc_step);
            const nfc = countSteps(pr.fc_min, pr.fc_max, pr.fc_step);
            const nDt = countSteps(pr.logDt_min, pr.logDt_max, pr.logDt_step);
            const nc  = countSteps(pr.c_min, pr.c_max, pr.c_step);
            let atoms = '?';
            if ([ntc, nfc, nDt, nc].every(x => typeof x === 'number')) {
                atoms = ntc * nfc * nDt * nc;
            }
            html += `
                <br><strong>Dictionary Params:</strong><br>
                tc (samples): [${fmt(pr.tc_min)} – ${fmt(pr.tc_max)}], step ${fmt(pr.tc_step)}<br>
                fc (Hz): [${fmt(pr.fc_min)} – ${fmt(pr.fc_max)}], step ${fmt(pr.fc_step)}<br>
                logDt: [${fmt(pr.logDt_min)} – ${fmt(pr.logDt_max)}], step ${fmt(pr.logDt_step)}<br>
                c (Hz/s): [${fmt(pr.c_min)} – ${fmt(pr.c_max)}], step ${fmt(pr.c_step)}<br>
            `;
            if (typeof pr.logDt_min === 'number' && typeof pr.logDt_max === 'number') {
                const msMin = 1000 * Math.exp(pr.logDt_min);
                const msMax = 1000 * Math.exp(pr.logDt_max);
                html += `duration_ms (approx from logDt): [${fmt(msMin, 2)} – ${fmt(msMax, 2)}] ms<br>`;
            }
            html += `<em>Grid steps:</em> tc×fc×logDt×c = ${ntc ?? '?'}×${nfc ?? '?'}×${nDt ?? '?'}×${nc ?? '?'} = <strong>${atoms}</strong> atoms<br>`;
        }
